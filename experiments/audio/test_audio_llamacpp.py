...
使用方法:
    python test_audio_llamacpp.py [--convert-only] [--test-only]
"""
import os
import sys
import subprocess
...
def test_audio_with_llamacpp(text_model_gguf=None):
    ...
    if not AUDIO_MMPROJ_GGUF.exists():
        ...
        print("请先运行转换: python test_audio_llamacpp.py --convert-only")
        return False
    ...
    if text_model_gguf is None or not Path(text_model_gguf).exists():
        ...
        print("请指定文本模型路径，或确保 artifacts/gguf/ 目录下有 gemma-3n-*.gguf 文件")
        return False
