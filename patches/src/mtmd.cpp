...
static std::string mtmd_trim(const std::string & s) {
...
static std::vector<std::string> mtmd_split_paths(const std::string & paths) {
    std::vector<std::string> out;
    std::string cur;
    for (char ch : paths) {
        if (ch == ',' || ch == ';') {
            auto t = mtmd_trim(cur);
            if (!t.empty()) {
...
struct mtmd_context {
        ...
        n_embd_text  (llama_model_n_embd_inp(text_model))
    {
        ...
        const std::vector<std::string> mmproj_paths = mtmd_split_paths(mmproj_fname);
        if (mmproj_paths.empty()) {
            throw std::runtime_error("mmproj path is empty");
        }

        auto load_mmproj = [&](const std::string & path) {
            auto res = clip_init(path.c_str(), ctx_clip_params);
            ...
            if (res.ctx_v) {
                if (ctx_v) {
                    clip_free(res.ctx_v);
                    throw std::runtime_error(string_format("vision mmproj already loaded, duplicate: %s\n", path.c_str()));
                }
                ctx_v = res.ctx_v;
            }
            if (res.ctx_a) {
                if (ctx_a) {
                    clip_free(res.ctx_a);
                    throw std::runtime_error(string_format("audio mmproj already loaded, duplicate: %s\n", path.c_str()));
                }
                ctx_a = res.ctx_a;
            }
        };

        for (const auto & path : mmproj_paths) {
            // stop early if both modalities are loaded
            if (ctx_v && ctx_a) {
                break;
            }
            load_mmproj(path);
    ...
    void init_audio() {
        ...
        } else if (proj == PROJECTOR_TYPE_GEMMA3NA) {
            // <start_of_audio> ... (embeddings) ... <end_of_audio>
            aud_beg = "<start_of_audio>";
    ...
    int32_t add_media(const mtmd_bitmap * bitmap) {
        if (!bitmap->is_audio) {
            ...
            for (auto & mel_spec : mel_spec_chunks) {
                ...
                // For all audio models: nx = time (n_len), ny = frequency (n_mel)
                // This matches the warmup convention in clip.cpp
                mel_f32->nx  = mel_spec.n_len; // time
                mel_f32->ny  = mel_spec.n_mel; // frequency
